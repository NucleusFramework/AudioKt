//! nucleus_rodio — hand-written JNI bridge over the Rust `rodio` crate for the
//! `dev.nucleusframework.rodio` Kotlin library. Symbols follow the package
//! `dev.nucleusframework.rodio.internal.NativeRodioBridge`.
//!
//! Callbacks (event / metadata / error) are delivered to a Kotlin
//! `NativePlaybackCallbackRaw` instance pinned via a `GlobalRef`. Rodio invokes
//! the ICY-metadata read on its internal decoder thread, so every callback
//! dispatch attaches the current thread to the JVM before calling back.
...
use jni::sys::{jboolean, jfloat, jint, jlong, JNI_FALSE, JNI_TRUE, JNI_VERSION_1_8};
...
use state::{register, unregister, with_player, with_player_mut, CallbackRef, PlayerState};

const MAX_SEEKABLE_HTTP_BYTES: u64 = 25 * 1024 * 1024; // 25MB cap to avoid buffering huge streams

/// JVM handle captured in `JNI_OnLoad`. Each `CallbackRef` also carries its own
/// `JavaVM` (taken from the `JNIEnv` of `nativeSetCallback`) so callbacks work
/// even if `JNI_OnLoad` has not been observed for some reason.
static JAVA_VM: OnceLock<JavaVM> = OnceLock::new();

/// Mirrors the ordinal of the Kotlin `PlaybackEvent` enum.
...
/// Throws `dev.nucleusframework.rodio.RodioException` with the error message.
/// The JVM raises the pending exception when the JNI export returns, so callers
/// must still return a safe default value.
fn throw(env: &mut JNIEnv, error: &RodioError) {
    let _ = env.throw_new("dev/nucleusframework/rodio/RodioException", error.to_string());
}

fn notify_event(callback: &Option<Arc<CallbackRef>>, event: PlaybackEvent) {
    let Some(cb) = callback else { return };
    let Ok(mut env) = cb.vm.attach_current_thread_permanently() else { return };
    let _ = env.call_method(
        cb.global.as_obj(),
        "onEvent",
        "(I)V",
        &[JValue::Int(event.as_int())],
    );
...
fn notify_error(callback: &Option<Arc<CallbackRef>>, error: &RodioError) {
    let Some(cb) = callback else { return };
    let Ok(mut env) = cb.vm.attach_current_thread_permanently() else { return };
    let Ok(msg) = env.new_string(error.to_string()) else { return };
    let _ = env.call_method(
        cb.global.as_obj(),
        "onError",
        "(Ljava/lang/String;)V",
...
fn notify_metadata(callback: &Option<Arc<CallbackRef>>, key: String, value: String) {
    let Some(cb) = callback else { return };
    let Ok(mut env) = cb.vm.attach_current_thread_permanently() else { return };
    let Ok(k) = env.new_string(key) else { return };
    let Ok(v) = env.new_string(value) else { return };
    let _ = env.call_method(
        cb.global.as_obj(),
        "onMetadata",
...
fn player_callback(id: u64) -> Result<Option<Arc<CallbackRef>>, RodioError> {
...
struct IcyMetadataReader<R: Read> {
    ...
    callback: Option<Arc<CallbackRef>>,
}

impl<R: Read> IcyMetadataReader<R> {
    fn new(inner: R, meta_interval: Option<usize>, callback: Option<Arc<CallbackRef>>) -> Self {
            ...
            if self.remaining_until_meta == 0 {
                ...
                if meta_len > 0 {
                    ...
                    for (key, value) in parse_icy_metadata_block(&metadata) {
                        notify_metadata(&self.callback, key, value);
                    }
                }
...
/// Reads a Kotlin `String` into a Rust `String`. On failure a JNI exception is
/// already pending, so callers should just return their safe default — the JVM
/// raises it when the export returns.
fn jstring(env: &mut JNIEnv, s: &JString) -> Option<String> {
    env.get_string(s).ok().map(|java_str| java_str.into())
}

/// JNI exports. Symbols follow `dev.nucleusframework.rodio.internal.NativeRodioBridge`.
/// Argument mapping: `u64 id` ↔ `jlong`; `bool` ↔ `jboolean`; `f32` ↔ `jfloat`;
/// `u32` ↔ `jint`; `String` ↔ `JString`; `Option<u64>` duration ↔ `jlong` (`-1` = None).

#[no_mangle]
pub extern "system" fn Java_dev_nucleusframework_rodio_internal_NativeRodioBridge_nativeCreatePlayer(
    mut env: JNIEnv,
    _class: JClass,
) -> jlong {
    match PlayerState::new() {
        Ok((player, stream)) => register(player, stream) as jlong,
        Err(e) => {
            throw(&mut env, &e);
            0
        }
...
#[no_mangle]
pub extern "system" fn Java_dev_nucleusframework_rodio_internal_NativeRodioBridge_nativeCreatePlayerWithBufferSizeFrames(
    mut env: JNIEnv,
    _class: JClass,
    buffer_size_frames: jint,
) -> jlong {
    // Treat negative ints as invalid (the Kotlin API surface is unsigned).
    let buffer_size_frames = buffer_size_frames.max(0) as u32;
    match PlayerState::new_with_buffer_size_frames(buffer_size_frames) {
        Ok((player, stream)) => register(player, stream) as jlong,
        Err(e) => {
            throw(&mut env, &e);
            0
...
#[no_mangle]
pub extern "system" fn Java_dev_nucleusframework_rodio_internal_NativeRodioBridge_nativeDestroyPlayer(
    mut env: JNIEnv,
    _class: JClass,
    id: jlong,
) {
    if let Err(e) = unregister(id as u64) {
        throw(&mut env, &e);
    }
}

#[no_mangle]
pub extern "system" fn Java_dev_nucleusframework_rodio_internal_NativeRodioBridge_nativeSetCallback(
    mut env: JNIEnv,
    _class: JClass,
    id: jlong,
    callback: JObject,
) {
    // Capture the VM from the calling JNIEnv (more robust than relying on JNI_OnLoad)
    // and pin the callback with a GlobalRef so it survives across native threads.
    ...
    let callback_ref = Arc::new(CallbackRef { global, vm });
    if let Err(e) = with_player_mut(id as u64, |state| {
        state.callback = Some(callback_ref);
        Ok(())
    }) {
        throw(&mut env, &e);
    }
...
#[no_mangle]
pub extern "system" fn Java_dev_nucleusframework_rodio_internal_NativeRodioBridge_nativeClearCallback(
    mut env: JNIEnv,
    _class: JClass,
    id: jlong,
) {
...
#[no_mangle]
pub extern "system" fn Java_dev_nucleusframework_rodio_internal_NativeRodioBridge_nativePlayFile(
    mut env: JNIEnv,
    _class: JClass,
    id: jlong,
    path: JString,
    ...
    let path = match jstring(&mut env, &path) {
        Some(path) => path,
        None => return,
    };
    let looped = looped != JNI_FALSE;
    let callback = match player_callback(id as u64) {
    ...
    let result = (|| {
        ...
        if looped {
            // `repeat_infinite` yields no samples on some decoders (e.g. symphonia/ogg);
            // `LoopedDecoder` seeks back to the start instead, which is also gapless.
            let decoder = builder.build_looped()?;
            with_player_mut(id as u64, |state| {
                reset_sink(state);
    ...
    match &result {
        Err(error) => notify_error(&callback, error),
        Ok(()) => notify_event(&callback, PlaybackEvent::Playing),
    }
    if let Err(e) = &result {
        throw(&mut env, e);
    }
...
#[no_mangle]
pub extern "system" fn Java_dev_nucleusframework_rodio_internal_NativeRodioBridge_nativePlaySine(
    mut env: JNIEnv,
    _class: JClass,
    id: jlong,
    frequency_hz: jfloat,
    ...
    if frequency_hz <= 0.0 {
        throw(&mut env, &RodioError::InvalidFrequency(frequency_hz));
        return;
    }
    if duration_ms == 0 {
        throw(&mut env, &RodioError::InvalidDuration(duration_ms));
        return;
    }
    let callback = match player_callback(id as u64) {
        Ok(callback) => callback,
    ...
    let result = with_player_mut(id as u64, |state| {
        reset_sink(state);
        state.current_duration = Some(duration);
        state.seekable = false;
        let source = SineWave::new(frequency_hz).take_duration(duration);
...
#[no_mangle]
pub extern "system" fn Java_dev_nucleusframework_rodio_internal_NativeRodioBridge_nativePlayUrl(
    mut env: JNIEnv,
    _class: JClass,
    id: jlong,
    url: JString,
    ...
    let url = match jstring(&mut env, &url) {
        Some(url) => url,
        None => return,
    };
    let looped = looped != JNI_FALSE;
    ...
    let result = (|| {
        if looped {
            ...
            return with_player_mut(id as u64, |state| {
                reset_sink(state);
                state.current_duration = None;
                state.sink.append(decoder);
                Ok(())
        ...
        if is_hls_playlist(&url, None) {
            return play_hls_stream(id as u64, &url);
        }
        ...
        if let Some(len) = response.content_length() {
            ...
            {
                let bytes = response.bytes()?;
                return play_seekable_http_bytes(id as u64, &url, bytes.to_vec(), content_type.as_deref());
            }
        }
...
#[no_mangle]
pub extern "system" fn Java_dev_nucleusframework_rodio_internal_NativeRodioBridge_nativePlayRadio(
    mut env: JNIEnv,
    _class: JClass,
    id: jlong,
    url: JString,
    ...
    let result = (|| {
        ...
        if is_hls_playlist(&final_url, content_type.as_deref()) {
            return play_hls_stream(id as u64, &final_url);
        }
        ...
        if is_playlist(&url, content_type.as_deref()) {
            ...
            if is_hls_playlist(&stream_url, None) {
                return play_hls_stream(id as u64, &stream_url);
            }
        ...
        for (key, header) in [
            ...
            if let Some(value) = header_value(&headers, header) {
                notify_metadata(&callback, key.to_string(), value);
            }
        }
...
#[no_mangle]
pub extern "system" fn Java_dev_nucleusframework_rodio_internal_NativeRodioBridge_nativePlay(
    mut env: JNIEnv,
    _class: JClass,
    id: jlong,
) {
    let result = with_player(id as u64, |state| {
        state.sink.play();
        Ok(state.callback.clone())
    });
    match result {
        Ok(callback) => notify_event(&callback, PlaybackEvent::Playing),
        Err(e) => throw(&mut env, &e),
    }
}

#[no_mangle]
pub extern "system" fn Java_dev_nucleusframework_rodio_internal_NativeRodioBridge_nativePause(
    mut env: JNIEnv,
    _class: JClass,
    id: jlong,
) {
    ...
    match result {
        Ok(callback) => notify_event(&callback, PlaybackEvent::Paused),
        Err(e) => throw(&mut env, &e),
    }
}

#[no_mangle]
pub extern "system" fn Java_dev_nucleusframework_rodio_internal_NativeRodioBridge_nativeStop(
    mut env: JNIEnv,
    _class: JClass,
    id: jlong,
) {
    ...
    match result {
        Ok(callback) => notify_event(&callback, PlaybackEvent::Stopped),
        Err(e) => throw(&mut env, &e),
    }
}

#[no_mangle]
pub extern "system" fn Java_dev_nucleusframework_rodio_internal_NativeRodioBridge_nativeClear(
    mut env: JNIEnv,
    _class: JClass,
    id: jlong,
) {
...
#[no_mangle]
pub extern "system" fn Java_dev_nucleusframework_rodio_internal_NativeRodioBridge_nativeIsPaused(
    mut env: JNIEnv,
    _class: JClass,
    id: jlong,
) -> jboolean {
    match with_player(id as u64, |state| Ok(state.sink.is_paused())) {
        Ok(true) => JNI_TRUE,
        Ok(false) => JNI_FALSE,
        Err(e) => {
            throw(&mut env, &e);
...
#[no_mangle]
pub extern "system" fn Java_dev_nucleusframework_rodio_internal_NativeRodioBridge_nativeIsEmpty(
    mut env: JNIEnv,
    _class: JClass,
    id: jlong,
) -> jboolean {
    match with_player(id as u64, |state| Ok(state.sink.empty())) {
        Ok(true) => JNI_TRUE,
        Ok(false) => JNI_FALSE,
        Err(e) => {
            throw(&mut env, &e);
...
#[no_mangle]
pub extern "system" fn Java_dev_nucleusframework_rodio_internal_NativeRodioBridge_nativeGetPositionMs(
    mut env: JNIEnv,
    _class: JClass,
    id: jlong,
) -> jlong {
    match with_player(id as u64, |state| Ok(duration_to_millis(state.sink.get_pos()))) {
        Ok(ms) => ms as jlong,
        Err(e) => {
            throw(&mut env, &e);
            0
...
#[no_mangle]
pub extern "system" fn Java_dev_nucleusframework_rodio_internal_NativeRodioBridge_nativeSeekPositionMs(
    mut env: JNIEnv,
    _class: JClass,
    id: jlong,
    position_ms: jlong,
...
#[no_mangle]
pub extern "system" fn Java_dev_nucleusframework_rodio_internal_NativeRodioBridge_nativeGetDurationMs(
    mut env: JNIEnv,
    _class: JClass,
    id: jlong,
) -> jlong {
    // `-1` signals "no duration" (None) to Kotlin.
    match with_player(id as u64, |state| Ok(state.current_duration.map(duration_to_millis))) {
        Ok(Some(ms)) => ms as jlong,
        Ok(None) => -1,
        Err(e) => {
            throw(&mut env, &e);
            -1
...
#[no_mangle]
pub extern "system" fn Java_dev_nucleusframework_rodio_internal_NativeRodioBridge_nativeIsSeekable(
    mut env: JNIEnv,
    _class: JClass,
    id: jlong,
) -> jboolean {
    match with_player(id as u64, |state| Ok(state.seekable)) {
        Ok(true) => JNI_TRUE,
        Ok(false) => JNI_FALSE,
        Err(e) => {
            throw(&mut env, &e);
...
#[no_mangle]
pub extern "system" fn Java_dev_nucleusframework_rodio_internal_NativeRodioBridge_nativeSetVolume(
    mut env: JNIEnv,
    _class: JClass,
    id: jlong,
    volume: jfloat,
    ...
    if volume < 0.0 {
        throw(&mut env, &RodioError::InvalidVolume(volume));
        return;
    }
    let result = with_player(id as u64, |state| {
...
#[no_mangle]
pub extern "system" fn Java_dev_nucleusframework_rodio_internal_NativeRodioBridge_nativeHttpSetAllowInvalidCerts(
    mut env: JNIEnv,
    _class: JClass,
    allow: jboolean,
) {
    let allow = allow != JNI_FALSE;
    let result = http_options()
        .lock()
        .map(|mut guard| {
...
#[no_mangle]
pub extern "system" fn Java_dev_nucleusframework_rodio_internal_NativeRodioBridge_nativeHttpAddRootCertPem(
    mut env: JNIEnv,
    _class: JClass,
    pem: JString,
) {
    let pem = match jstring(&mut env, &pem) {
        Some(pem) => pem,
        None => return,
    };
    let result = (|| {
...
#[no_mangle]
pub extern "system" fn Java_dev_nucleusframework_rodio_internal_NativeRodioBridge_nativeHttpClearRootCerts(
    mut env: JNIEnv,
    _class: JClass,
) {
    let result = http_options()
