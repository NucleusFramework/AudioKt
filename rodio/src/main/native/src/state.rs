...
/// A Kotlin `NativePlaybackCallbackRaw` instance pinned across threads.
///
/// The `GlobalRef` keeps the JVM-side callback alive and the `JavaVM` lets us
/// attach the current (possibly non-JVM) thread — rodio invokes callbacks from
/// its internal decoder/audio thread, which is never attached by default.
...
pub struct PlayerState {
    pub sink: Sink,
    pub callback: Option<Arc<CallbackRef>>,
